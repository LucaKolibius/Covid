...
% % data to april 5
% load 'C:\Users\Luca\Downloads\Covid-19-master\data\data_all_to_April_5'

addpath(genpath('C:\Users\Luca\Downloads\spm12_latest\spm12'));
clear
close all
importNewTS  % most up to date TS
allTW = [40 50 60 70 80 89];  % 75 is 6th of april
countries_test = {'Ireland','United Kingdom','Italy','Spain','France','Germany','Switzerland'};
tic
for ix = 1:6 % for each TW
    clearvars -except allTW ix TS_death TS_infec countries_hubei countries_test
    
    % cut out relevant time window
    TW = allTW(ix);
    data_all.deaths_diff = TS_death(:,1:TW);
    data_all.cases_diff  = TS_infec(:,1:TW);
    
    for countries = 1:size(countries_test,2)
        % select gaussian model priors
        ...
        for act = 1:10 % Actualization of priors with posteriors
            
            % uses the posterior as a new prior if there is a posterior
            if exist('Ep','file')
                pE = Ep;
            end
    ...
    save(['Kolibius_GCM_1_7_v', num2str(ix)], 'GCM');
end
%
